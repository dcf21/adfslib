        ...
        try:
            ...
            for element in elements:
                ...
                if not os.path.exists(built):
                    ...
                    os.mkdir(built)
                    logging.debug('Created directory <{}>'.format(built))

                elif not os.path.isdir(built):

                    # This element of the directory already exists
                    # but is not a directory.
                    logging.warning('A file exists which prevents a directory from being created: <{}>'.format(built))

                    return ""

        except OSError:

            logging.warning('Directory could not be created: <{}>'.format(string.join(elements, os.sep)))
        ...
        while self.sectors[head + p] != 0:
            ...
            if inddiscadd == -1:
                ...
                if (newdiratts & 0x8) != 0:
                    ...
                    files.append(ADFSfile(name, bytes(), load, exe, length))
...
class ADFSdisc(Utilities):
    ...
    def __init__(self, adf, filename: str = "", verify=0, assume_interleaved=1):
        ...
        if self.disc_type == 'adD':
            ...
            self.root_name, self.files = self.disc_map.read_catalogue(
                (self.ntracks * self.nsectors / 2 + 2) * self.sector_size)
    ...
    def _read_disc_record(self, offset):
        ...
        bit_size = self._read_unsigned_byte(self.sectors[offset + 6])
    ...
    def _read_tracks(self, f, inter):
        ...
        if inter == 0:
            try:
                ...
                logging.warning('Fewer than {:d} tracks found.'.format(self.ntracks))
                f.close()
                raise ADFS_exception('Fewer than {:d} tracks found.'.format(self.ntracks))
    ...
    def _read_old_catalogue(self, base):
        ...
        dir_seq = self.sectors[head + p]
        dir_start = self._safe(self.sectors[head + p + 1:head + p + 5])
        if dir_start not in self.dir_markers:
            ...
            if isinstance(obj, ADFSfile):
                ...
                if not filetypes:
                    ...
                    try:
                        ...
                        logging.error("Couldn't open the file: <{}>\n{}".format(out_file, traceback.format_exc()))
                    ...
                    try:
                        ...
                        logging.error("Couldn't open the file: <{}>\n{}".format(inf_file, traceback.format_exc()))
